...
then
    ...
    velero backup create scheduledworkflows-kubeflow-org-backup --include-resources scheduledworkflows.kubeflow.org
    velero backup create workflows-argoproj-io-backup --include-resources workflows.argoproj.io
    velero backup create cronworkflows-argoproj-io-backup --include-resources cronworkflows.argoproj.io
    velero backup create viewers-kubeflow-org-backup --include-resources viewers.kubeflow.org
    velero backup create all-crds-backup --include-resources customresourcedefinitions.apiextensions.k8s.io
    velero backup create all-storageclass-backup --include-resources storageclasses.storage.k8s.io
    velero backup create all-priorityclass-backup --include-resources priorityclasses.scheduling.k8s.io
    ...
    for i in `cat $file`
    do
        velero backup create "$i"-ns-backup --include-namespaces $i
    done
